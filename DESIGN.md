...
**Korean is served by exactly one webfont family, deliberately.** Google's Korean faces ship ~2,500 glyphs across ~94 unicode-range subsets, so a second Korean family would roughly double both the @font-face count and the bytes a Korean page pulls. Plex Sans KR therefore does both display and body duty, and Fraunces stays Latin-only so it never pulls a Korean download.

**That family is loaded with `preload: false`, and it is the single largest performance decision on the site.** Three weights of Plex Sans KR emit 282 `@font-face` rules, and `next/font` preloads all of them — `subsets: ['latin']` does not narrow it. A `<link rel=preload>` is unconditional, so it defeats the very unicode-range mechanism that keeps a Korean page from downloading all of Korean: every route was shipping 187 preload tags and ~1.8 MB of woff2, about three quarters of its total weight, with the tags alone adding ~48 KB to the HTML. With preloading off the browser fetches only the subsets the page's glyphs land in — 12–44 files, 157–393 KB by route — and `display: swap` keeps the fallback painting meanwhile. Measured on the home page: 2.4 MB → 0.85 MB, 241 requests → 62.
...
Mono carries 600 even though the weight budget is otherwise tight. Measured cost: +32 KB on disk across five unicode-range subsets, of which a page downloads only `latin` — against what a Korean weight would add (see the preload note above). It earns that by carrying every emphasised mono label: table headers, status chips, stat values.
