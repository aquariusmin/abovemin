...
마지막으로 `supabase/migrations/20260917010000_upload_privacy.sql`을 적용합니다(archive-extras **다음**). `photos.gps_in_original boolean`(nullable) 하나를 더합니다 — 촬영 정보 채우기가 원본 메타데이터에 GPS·장소 태그가 남아 있으면 true, 없으면 false를 적고, null은 아직 모름입니다. 적용 전에는 채우기가 이 표시만 빼고 계속 동작하고, 개요에 "마이그레이션 적용 필요"가 뜹니다.
