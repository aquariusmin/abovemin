...
- **라이트박스** — 촬영 정보가 있으면 한 줄("2019.07.31 · iPhone 7 · 4mm · f/1.8 · 1/1053s · ISO 20")과, **이 사진으로 프린트 문의** 링크(`mailto:`, 제목과 본문에 사진 캡션과 사진 페이지 링크). "링크 복사"도 사진 페이지 주소를 복사합니다. 예전에 건넨 `?p=` 링크는 계속 앨범에서 라이트박스를 엽니다.
- **사진 페이지 `/archive/[slug]/[id]`** — 사진 한 장, 촬영일·장소·카메라·설정·컬렉션, 링크 복사와 프린트 문의, 앨범 안의 이전/다음(←/→). 링크 미리보기는 사진을 자르지 않고 1200×630 종이색 여백에 얹은 Cloudinary 이미지입니다. 숨긴 사진, 비공개 앨범, 다른 앨범의 주소는 404이고, sitemap에 실립니다. 카메라를 누르면 `/archive?camera=…`로 그 카메라 사진만 걸러 봅니다.
- **타임라인 `/archive/timeline`** — 촬영일 기준 연도 → 달, 촬영일이 없으면 그해의 "날짜 미상". `/archive`의 "전체에서 찾기" 옆 버튼으로 갑니다.
- **홈 "몇 년 전 오늘"** — 서울 날짜로 오늘과 월·일이 같은 사진(없으면 앞뒤 사흘, 최대 6장). 맞는 사진이 없으면 섹션이 없습니다. 홈의 ISR 60초 주기에 날짜가 따라갑니다.
- **앨범 설명** — 관리 화면의 앨범 "설명"이 앨범 페이지 제목 아래와 검색 설명에 나옵니다. 비어 있으면 줄이 없습니다.
- **그리드** — 크기가 저장된 사진은 이미지가 오기 전에 비율만큼 자리를 잡고, 그 칸에 흐린 32px 미리보기를 깝니다(크기 자체가 아니라 비율만 씁니다).
