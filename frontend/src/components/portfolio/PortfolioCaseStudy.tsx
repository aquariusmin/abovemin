    ...
    <main lang={isKorean ? "ko" : "en"} className="portfolio-ui min-h-screen bg-surface px-4 py-10 font-sans text-[#222] sm:px-6 md:px-10 md:py-16">
      <article className="mx-auto max-w-5xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link href={overviewPath} className={`text-[10px] font-bold text-gray-400 transition-colors hover:text-accent ${isKorean ? "tracking-[0.18em]" : "uppercase tracking-[0.25em]"}`}>
            &larr; {mode === "submission" && !isKorean ? "Submission " : ""}{mode === "submission" && isKorean ? "제출용 " : ""}{copy.back}
          </Link>
          <div className="flex flex-wrap items-center justify-end gap-x-4 gap-y-2">
            {mode === "submission" && (
              <>
                <a href="mailto:aquariusmin01@naver.com" className="text-[10px] font-bold text-gray-400 transition-colors hover:text-accent">
                  Email
                </a>
                <a href="https://github.com/aquariusmin" target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold text-gray-400 transition-colors hover:text-accent">
                  GitHub
                </a>
                <Link href={getPortfolioPrintPath(locale, mode)} className="text-[10px] font-bold text-gray-400 transition-colors hover:text-accent">
                  {copy.print}
                </Link>
              </>
            )}
            <Link href={getPortfolioCasePath(alternateLocale, mode, project.slug)} hrefLang={alternateLocale} className={`text-[10px] font-bold text-gray-400 transition-colors hover:text-accent ${isKorean ? "uppercase tracking-[0.2em]" : "tracking-[0.16em]"}`}>
        ...
        <header className="mt-10 space-y-8 border-b border-black/5 pb-12 md:mt-14 md:pb-16">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-accent">Case {project.number}</span>
            <span className={`text-[10px] font-semibold text-gray-400 ${isKorean ? "tracking-[0.1em]" : "uppercase tracking-[0.2em]"}`}>{project.category}</span>
          </div>
          <div className="space-y-5">
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl md:text-6xl">{project.title}</h1>
            <p className="max-w-3xl break-keep text-base leading-relaxed text-gray-600 md:text-lg">{project.summary}</p>
          </div>
          <div className="grid gap-4 border-y border-black/5 py-5 text-sm text-gray-600 sm:grid-cols-2">
        ...
        <div className="space-y-16 py-14 md:space-y-24 md:py-20">
          <CaseSection label={copy.sections[0][0]} title={copy.sections[0][1]} isKorean={isKorean}>
            <p className="break-keep border-l-4 border-accent bg-white/55 px-6 py-7 text-xl font-semibold leading-relaxed md:px-8 md:py-9 md:text-2xl">{project.question}</p>
          </CaseSection>

          <CaseSection label={copy.sections[1][0]} title={copy.sections[1][1]} isKorean={isKorean}>
            <div className="grid gap-4 md:grid-cols-3">
              {project.evidence.map((item) => <div key={item} className="break-keep border border-black/5 bg-white/55 p-5 text-sm leading-relaxed text-gray-600">{item}</div>)}
            </div>
            <div className="mt-5 flex flex-wrap gap-2">
              {project.tools.map((tool) => <span key={tool} className="bg-surface-muted px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-600">{tool}</span>)}
            </div>
          </CaseSection>
          ...
          <CaseSection label={copy.sections[3][0]} title={copy.sections[3][1]} isKorean={isKorean}>
            <div className="grid gap-4 md:grid-cols-2">
              {project.insights.map((insight) => <p key={insight} className="break-keep border border-black/5 bg-white/55 p-6 text-sm leading-relaxed text-gray-700">{insight}</p>)}
            </div>
          ...
          <CaseSection label={copy.sections[4][0]} title={copy.sections[4][1]} isKorean={isKorean}>
            <p className="max-w-4xl break-keep text-2xl font-semibold leading-relaxed text-accent md:text-3xl">{project.decisionValue}</p>
          </CaseSection>
        ...
        <nav aria-label={isKorean ? "프로젝트 이동" : "Case study navigation"} className="grid gap-px border-y border-black/5 bg-black/5 sm:grid-cols-2">
          <Link href={getPortfolioCasePath(locale, mode, previous.slug)} className="group bg-surface p-6 transition-colors hover:bg-white md:p-8">
            <span className={`text-[9px] font-bold text-gray-400 ${isKorean ? "tracking-[0.12em]" : "uppercase tracking-[0.2em]"}`}>{copy.previous}</span>
          ...
          <Link href={getPortfolioCasePath(locale, mode, next.slug)} className="group bg-surface p-6 text-right transition-colors hover:bg-white md:p-8">
            <span className={`text-[9px] font-bold text-gray-400 ${isKorean ? "tracking-[0.12em]" : "uppercase tracking-[0.2em]"}`}>{copy.next}</span>
  ...
  return (
    <section>
      <div className="mb-7 space-y-2">
        <p className={`text-[10px] font-bold text-gray-400 ${isKorean ? "tracking-[0.16em]" : "uppercase tracking-[0.3em]"}`}>{label}</p>
