  ...
  return (
    <section className="grid gap-8 border-y border-hairline py-12 md:grid-cols-12 md:items-center md:py-16">
      ...
      <div className="flex flex-col items-start gap-3 text-sm font-semibold md:col-span-5 md:items-end">
        <a className="link-underline text-accent" href="mailto:aquariusmin01@naver.com">aquariusmin01@naver.com</a>
        <a className="link-underline text-accent" href="https://github.com/aquariusmin" target="_blank" rel="noopener noreferrer">github.com/aquariusmin</a>
        {mode === "normal" && (
          <Link className="link-underline text-accent" href="/about">{isKorean ? "소개 더 보기" : "More about me"} &rarr;</Link>
        )}
        <Link className="link-underline text-accent" href={getPortfolioPrintPath(locale, mode)}>{isKorean ? "인쇄 · PDF로 저장" : "Print / Save as PDF"} &rarr;</Link>
      </div>
    </section>
  );
}
